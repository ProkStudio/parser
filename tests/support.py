"""Synthetic account/data adapter for tests only. Never connects to Telegram."""
import asyncio
...
class FakeGateway:
    ...
    async def auth(self, action, data):
        ...
        elif action == "verify_code" and data.get("code") == "22222":
            self.state["step"] = "password"
        elif action in ("verify_code", "verify_password"):
